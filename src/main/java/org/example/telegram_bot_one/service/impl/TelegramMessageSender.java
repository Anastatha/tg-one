...
public class TelegramMessageSender implements IMessageSender {
    ...
    private boolean sendText(MessageTask task) {
        ...
        try {
            ...
            Integer retrySeconds = handleRateLimit(e);
            if (retrySeconds != null) {
                throw new FloodWaitException(retrySeconds);
            }
            return false;
        } catch (Exception e) {
    ...
    private Integer handleRateLimit(TelegramApiRequestException e) {
        ...
        if (response != null && response.contains("429")) {
            try {
                ...
                if (m.find()) {
                    int retrySeconds = Integer.parseInt(m.group(2));
                    log.warn("Telegram global FLOOD_WAIT={}s", retrySeconds);
                    return retrySeconds;
                }
